...
class Session {
  public:
    ...
    struct frameQueueItem {
        ...
        explicit frameQueueItem(std::shared_ptr<std::valarray<uint8_t>> bytes, int rowPitch)
            : data(std::move(bytes)), rowPitch(rowPitch) {}
    ...
    HRESULT enqueueVideoFrame(BYTE* pData, int rowPitch, int size);
